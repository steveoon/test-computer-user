...
const suggestions = [
  ...
  {
    text: "Boss直聘智能回复（集成版）",
    prompt: `我在Boss直聘消息页面，请使用集成的自动回复功能处理对话：
    1) 先截图查看当前页面状态；
    2) 在左侧聊天列表中找到有红色圆点未读消息提示的后续人，点击进入对话；
       - 点击头像后，你会看到对方的信息，包括头像和姓名等
    3) 再次截图查看聊天内容，仔细阅读对方最新的消息；
    3) 根据截图中的聊天内容，调用 generate_zhipin_reply 进行回复
    4) 请在生成回复后暂停，让我确认内容是否合适，然后再手动发送。`,
  },
  {
